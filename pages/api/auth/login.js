...
export default async function handler(req, res) {
  ...
  const { data: inProgressA }= await db.from('assessments').select('id, last_question_index').eq('user_id', user.id).eq('status', 'in_progress').maybeSingle()
  ...
  return res.status(200).json({
    user: {
      ...
      email:                   user.email,
      stage:                   user.grade || null,   // grade column stores stage
      emailVerified:           user.email_verified || false,
      hasCompletedPayment:     !!payment,
      hasCompletedAssessment:  !!completedA,
      hasInProgressAssessment: !!inProgressA,
      inProgressQuestionIndex: inProgressA?.last_question_index || 0,
